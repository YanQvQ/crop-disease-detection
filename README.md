<div align="center" style="margin: 36px auto 16px;">
  ...
  <div style="margin: 18px auto 0; max-width: 430px; display: flex; align-items: center; gap: 14px; font-family: 'SF Mono', 'JetBrains Mono', Consolas, monospace; font-size: 0.72rem; letter-spacing: 0.14em; opacity: 0.55;">
    ...
    <span>Flask &middot; SQLite &middot; JWT &middot; v2.3.0</span>
    <span style="flex: 1; height: 1px; background: rgba(128, 128, 128, 0.3);"></span>
  </div>
...
- **治疗建议**：内置病害知识库，针对检测到的病害自动展示病害解释、发病症状、防治方法与用药建议
- **历史记录结果展示**：图片/视频/摄像头记录在列表与详情中同步展示识别结果、置信度与防治建议，便于回查
- **视频记录缩略图**：视频检测记录自动抽取关键帧生成缩略图，列表一目了然、点击直达详情
- **低置信度预警**：置信度低于 80% 时提示结果可能存在偏差，逐项标注"建议核实"并引导联系当地农技专家
- **未检出提示**：未检测到病害时区分作物健康/检出失败与所选模型不匹配两种情况，建议确认模型后重试，并结合田间实地情况综合判断
- **用药免责警示**：防治建议中的用药信息附橙黄色警示样式，注明请以当地农技专家或专业人士意见为准
- **多目标归并**：同类病害自动合并展示（×N），按最高置信度归并结果
- **置信度显示开关**：支持四舍五入（整数百分比）与精确百分比切换，统一各页面"最高置信度"术语与格式
...
│
├── static/                 # 静态资源
│   ├── crop_diseases.json  # 病害知识库（防治建议/用药免责等）
│   ├── css/
│   │   ├── common.css      # 全局样式
│   │   └── sidebar.css     # 侧边栏样式
│   └── js/
│       └── common.js       # 公共工具库（置信度格式化/病害卡渲染等）
│
├── files/                  # 上传文件存储（自动生成）
├── runs/                   # 检测结果临时目录（自动生成）
└── app.log                 # 运行日志（自动生成）
...
### 病害知识库（crop_diseases.json）如何维护

检测结果展示的病害解释、发病症状、防治方法与用药建议均来自 `static/crop_diseases.json`。文件按作物分组，每条病害以键值对存储，**病害 `key` 必须与 YOLO 模型输出的类别名一致（不区分大小写）**，否则检测到该病害时找不到对应资料，会提示"未收录该病害的详细防治资料，建议结合田间实地情况，咨询当地农技专家、农业部门或专业人士进一步确认。如需更详细的防治信息，可联系管理员补充该病害资料"。

```jsonc
{
  "crops": {
    "corn": {                        // 作物 key，名称任意（前端仅按病害 key 查找）
      "crop_zh": "玉米",
      "diseases": {
        "blight": {                  // ← 必须等于模型类别名（如 blight / common_rust）
          "name_zh": "玉米大斑病",
          "name_en": "Corn Leaf Blight",
          "pathogen": "病原……",
          "transmission": "传播……",
          "harm_level": { "stars": 3, "label": "中", "yield_loss": "减产 20%~50%" },
          "symptoms": "识病要点：梭形大斑……",     // 页面「识病要点」
          "conditions": "发病条件……",            // 页面「发病条件」
          "detection_hint": "识别提示……",
          "season": "7~9 月",
          "region": "……",
          "similar_diseases": ["common_rust", "gray_spot"],
          "treatment": [                          // 页面「防治建议」
            { "type": "化学防治", "detail": "……" },
            { "type": "农业防治", "detail": "……" }
          ]
...
1. **新增病害**：在对应作物的 `diseases` 下增加一个键，`key` 写入模型类别名，至少填写 `name_zh`、`symptoms`、`conditions`、`treatment` 四个字段。
2. **修改内容**：直接编辑对应字段的中文描述、`harm_level` 等级与减产损失、`treatment` 防治条目即可。
3. **模型换名**：若新模型类别名与现有 `key` 不一致，修改 `key`（或新增别名）以保持匹配。
4. **当前页面仅展示 3 个字段**：`symptoms`（识病要点）、`conditions`（发病条件）、`treatment`（防治建议）。`pathogen / transmission / harm_level / detection_hint / season / region / similar_diseases` 等字段 JSON 中已预置，但**前端暂不渲染**；如需展示版权重/发生季节等，需同时修改 `static/js/common.js` 中 `_diseaseCardHtml` 的渲染逻辑。
5. **改后清除缓存**：前端固定路径拉取本文件、无版本号，修改后浏览器可能出现旧内容，需强制刷新（Ctrl+F5）或给 `static/crop_diseases.json` 的请求加版本号。
6. **同步元信息**：改完请同步更新 `meta.version` 与 `meta.updated_at`。
...
- ✅ 新增视频记录缩略图：视频记录自动抽帧生成缩略图，列表卡片展示、点击直达详情
- ✅ 修复仪表盘横向溢出与布局错乱问题
- ✅ 修复浏览器缓存旧静态资源导致的显示异常
- ✅ 修复历史记录跨主机加载图片被拦截（ERR_BLOCKED_BY_ORB）与视频"非法的视频地址"问题
- ✅ 修复分页翻页、删除后列表不刷新问题
- ✅ 优化通知中心：每次打开即时拉取最新通知，系统更新通知置顶
- ✅ 同步系统版本号至 2.3.0
- ✅ 病害知识库维护入口：系统设置中新增维护提示；检测到未收录病害时，普通用户提示"可联系管理员补充该病害资料"，管理员额外可见"可在 static/crop_diseases.json 新增资料"的管理提示
